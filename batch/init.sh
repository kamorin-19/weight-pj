...
# 指定したデータベース内に「daily_calories」というテーブルが存在するかチェック
echo "Checking if table 'daily_calories' exists in database 'weight_admin'..."
TABLE_EXISTS=$(mysql -u root -p"rootpassword" -D "weight_admin" -e "SHOW TABLES LIKE 'daily_calories';" | grep daily_calories || true)

# カロリー記録テーブルが存在しない場合は作成
if [ -z "$TABLE_EXISTS" ]; then
    echo "Table 'daily_calories' not found. Creating table..."
    mysql -u root -p"rootpassword" -D "weight_admin" < /docker-entrypoint-initdb.d/create_daily_calories.sql
else
